...
class MemcacheSessionStore extends SessionStore {
	...
	public void configureSessionStore(CommandLineParams commandLineParams, Context ctx) {
		...
		else if (System.getenv("MEMCACHIER_SERVERS") != null) { // Using
																// Memcachier
		...
		manager.setSessionBackupTimeout(commandLineParams.sessionStoreOperationTimout);
	...
	private void printNoConfigError() {
		System.out
				.println("WARNING: memcache session store being used, but the required environment variables aren't set.");
		System.out
				.println("Memcache session store is configured with MEMCACHE_SERVERS, MEMCACHE_USERNAME, MEMCACHE_PASSWORD");
